...
class HelpMenu:
    ...
    def get_menu(self) -> str:
        result = f"**Mystic Code:** `{self.filename}`"
        if self.command_info:
            result += f"\n**Archive Note:** __{self.command_info} 🍀__"
        result += "\n\n"
        for command in self.command_dict:
            command = self.command_dict[command]
            result += f"**{Symbols.radio_select} Spell:** `{Config.HANDLERS[0]}{command['command']}"
            ...
            if command["description"]:
                result += (
                    f"**{Symbols.arrow_right} Effect:** __{command['description']}__\n"
                )
            if command["example"]:
                result += f"**{Symbols.arrow_right} Incantation:** `{Config.HANDLERS[0]}{command['example']}`\n"
            if command["note"]:
...
class BotHelp:
    ...
    def get_menu(self) -> str:
        result = f"**Archive Category:** `{self.category}`"
        if self.command_info:
            result += f"\n**Archive Note:** __{self.command_info}__"
        result += "\n\n"
        for command in self.command_dict:
            command = self.command_dict[command]
            result += f"**{Symbols.radio_select} Spell:** `/{command['command']}`\n"
            if command["description"]:
                result += (
                    f"**{Symbols.arrow_right} Effect:** __{command['description']}__\n"
                )
