...
class Database:
    def __init__(self, uri: str) -> None:
        ...
        self.master_users = self.db["master_users"]

    async def connect(self):
        try:
            await self.client.admin.command("ping")
            LOGS.info(
                f"{Symbols.bullet * 3} Root archive connected {Symbols.bullet * 3}"
            )
    ...
    async def is_master(self, client: int, user_id: int) -> bool:
        if await self.master_users.find_one({"client": client, "user_id": user_id}):
            return True
        return False

    async def add_master(self, client: int, user_id: int) -> bool:
        if await self.is_master(client, user_id):
            return False
        await self.master_users.insert_one(
            {"client": client, "user_id": user_id, "date": self.get_datetime()}
        )
        return True

    async def rm_master(self, client: int, user_id: int) -> bool:
        if not await self.is_master(client, user_id):
            return False
        await self.master_users.delete_one({"client": client, "user_id": user_id})
        return True

    async def get_masters(self, client: int) -> list:
        return [i async for i in self.master_users.find({"client": client})]

    async def get_all_masters(self) -> list:
        return [i async for i in self.master_users.find({})]
